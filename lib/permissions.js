module.exports = {
     ...
     */
    check_permissions: function(permission_handle, member) {
    ...
    check_permissions_slash : function(permission_handle, member) {
        //Parse config.json
        const fs = require('fs');
        var data = fs.readFileSync('./config.json');
        var config = JSON.parse(data);
        ...
        if (config["commandPermissions"][permission_handle] && config["commandPermissions"][permission_handle].length) {
            for (var index in config["commandPermissions"][permission_handle]) {
                ...
                //Member has permission
                if (member['roles'].includes(config["commandPermissions"][permission_handle][index]["roleId"])) {
                    return true;
                }
                if(parseInt(index, 10) === config["commandPermissions"][permission_handle].length-1) {
                    return false;
